...
const VerPendientes = (props) => {
    ...
    const generarDatos = () =>{
        ...
        let teams  = equipos.filter(x=>x.torneo == props.idTorneo && x.status == 'alta')
        let juegosTorneo  = juegos.filter(x=>x.torneo == props.idTorneo)
        ...
        teams.forEach(element => {
            ...
            let equiposTorneo  = equipos.filter(x=>x.torneo == props.idTorneo 
                && x.status == 'alta' 
                && x.id != element.id)
            ...
            for(let i=0; i < equiposTorneo.length; i++){
                ...
                console.log (yasejugoA)
                console.log(yasejugoB)
                console.log('')
    ...
    return (
        <>
            ...
            <tbody>
                ...
                {
                    equipos.filter(x=>x.torneo == props.idTorneo && x.status == 'baja')
                    .map((item,ndx) => (
                        <tr key={ndx}  >
                            <td>{ndx + 1}  </td>                        
                            <td>{item.nombre} </td>                                                                                    
